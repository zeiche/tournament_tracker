#!/usr/bin/env python3
"""Center table and reduce vertical padding"""
import os
import requests
import json
...
print("Creating centered table with compact spacing...")
...
# Build HTML with centering and minimal padding
html = f"""<div style="text-align: center; margin: 0 auto;">
<h1>Player Rankings - {datetime.now().strftime('%B %d, %Y')}</h1>
<p>Points: 1st=10, 2nd=7, 3rd=5, 4th=3, 5th-6th=2, 7th-8th=1</p>
<table style="margin: 0 auto; border-collapse: collapse;">
<tr>
<th style="padding: 4px 10px;">Rank</th>
<th style="padding: 4px 10px;">Player</th>
<th style="padding: 4px 10px;">Points</th>
<th style="padding: 4px 10px;">Events</th>
<th style="padding: 4px 10px;">1st Places</th>
</tr>"""
...
for i, p in enumerate(player_stats, 1):
    ...
    # Minimal padding - 4px vertical, 10px horizontal
    html += f"""<tr><td style="padding: 4px 10px;">{i}</td><td style="padding: 4px 10px;">{tag}</td><td style="padding: 4px 10px;">{pts}</td><td style="padding: 4px 10px;">{evts}</td><td style="padding: 4px 10px;">{wins}</td></tr>"""
    ...
    if template_response.status_code == 200:
        ...
        try:
            ...
            if update_response.status_code in [200, 201]:
                print(f"✅ Updated with centered table and reduced padding")
                print(f"   - Table centered with margin: 0 auto")
                print(f"   - Padding reduced to 4px vertical, 10px horizontal")
